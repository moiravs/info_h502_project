...
public:
    static std::shared_ptr<InstancedObject> make(const char *path, const std::string &shader, const std::vector<glm::mat4>& models);
    InstancedObject(const char *path, const std::shared_ptr<Renderer>& renderer);
