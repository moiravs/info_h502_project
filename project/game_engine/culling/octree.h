...
class Octree : public std::enable_shared_from_this<Octree> {
    ...
    std::vector<std::shared_ptr<RenderableEntity>> data {};
    glm::vec3 center;
    glm::vec3 halfDim;
    glm::vec3 minBound;
    glm::vec3 maxBound;
    ...
    void put(const std::vector<std::shared_ptr<RenderableEntity>>& objects, const glm::vec3& position);
    void moveObjectUp(const std::shared_ptr<RenderableEntity>& object, const glm::vec3& position);
    std::pair<bool, std::vector<std::shared_ptr<RenderableEntity>>> childrenAllLeafAndSmashable() const;
